...
namespace NReflect
{
    ...
    internal class ReflectionWorker
    {
        ...
        public NRAssembly Reflect(string fileName)
        {
            ...
            var assemblyDirectory = Path.GetDirectoryName(fileName);
            if (assemblyDirectory != null)
            {
                paths.AddRange(Directory.GetFiles(assemblyDirectory, "*.dll"));
            }

            var resolver = new IgnoreAssemblyResolver(paths, _errors);
        ...
        private void ReflectType(Type type, IEntityContainer entityContainer)
        {
            ...
            {
                _errors.Add($"Failed to reflect type '{type.Name}'. {ex.Message}");
                return;
            }
        ...
        private void ReflectGenericType(Type type, NRGenericType nrGenericType)
        {
            ...
            nrGenericType.GenericTypes.AddRange(
                GetTypeParameters(type.GetGenericArguments().Skip(parentGenericArgsCount)));
        }
        ...
        private void ReflectParameters(MethodBase methodBase, List<NRParameter> nrParameters)
        {
            ...
            {
                _errors.Add($"Failed to reflect parameters of method '{methodBase.Name}'. {ex.Message}");
            }
        ...
        private void ReflectOperator(MethodInfo methodInfo, NRSingleInheritanceType singleInheritanceType)
        {
            ...
            nrOperator.OperatorType = operatorMethodsMap.ContainsKey(methodName)
                ? operatorMethodsMap[methodName]
            ...
            IMethodContainer methodContainer)
        {
            ...
            {
                _errors.Add($"Failed to reflect return type of method '{methodInfo.Name}'. {ex.Message}");
            }

            if (returnType != null)
            {
                nrOperation.Type = GetTypeUsage(returnType, methodInfo);
                nrOperation.TypeFullName = returnType.FullName ?? returnType.Name;
            }
            else
            {
                nrOperation.Type = new NRTypeUsage();
                nrOperation.TypeFullName = "__Unresolved__";
            }
            ...
            nrOperation.ReturnValueAttributes.AddRange(
                GetAttributes(CustomAttributeData.GetCustomAttributes(methodInfo)));
            ...
            if (dynamicAttributeData != null)
            {
                ...
                else
                {
                    ReadOnlyCollection<CustomAttributeTypedArgument> arguments =
                        (ReadOnlyCollection<CustomAttributeTypedArgument>)dynamicAttributeData.ConstructorArguments[0]
                            .Value;
            ...
            List<Type> genericArguments)
        {
            ...
            if (type.DeclaringType != null && !type.IsGenericParameter)
            {
                ...
                bool declaringTypeUsesOnlyGenericParam = genericArguments.Take(declaringTypeGenericArgsCount)
                    .All(t => t.IsGenericParameter);

                if (!sameDeclaringType || !declaringTypeUsesOnlyGenericParam)
                {
                    declaringTypeUsage = GetTypeUsage(type.DeclaringType, ref dynamicAttributeData, currentType,
                        genericArguments);
            ...
            NRTypeUsage nrTypeUsage = new NRTypeUsage
            {
                ...
                FullName = type.FullName ??
                           ((string.IsNullOrWhiteSpace(type.Namespace) ? "" : type.Namespace + ".") + type.Name)
            };
            ...
            if (type.GetGenericArguments().Length > 0)
            {
                ...
                {
                    nrTypeUsage.GenericParameters.Add(GetTypeUsage(genericArguments[0], ref dynamicAttributeData,
                        currentType, null));
        ...
        private IEnumerable<NRAttribute> GetAttributes(IEnumerable<CustomAttributeData> attributeDatas)
        {
            ...
            foreach (CustomAttributeData attributeData in attributeDatas)
            {
                ...
                if (attributeType != null && (attributeType == typeof(DynamicAttribute) ||
                                              ...
                                              attributeType == typeof(ParamArrayAttribute) ||
                                              attributeType == typeof(OptionalAttribute)))
                {
                    continue;
                }
        ...
        private IEnumerable<NRTypeParameter> GetTypeParameters(IEnumerable<Type> genericArguments)
        {
            ...
            foreach (Type genericArgument in genericArguments)
            {
                ...
                foreach (Type genericParameterConstraint in genericParameterConstraints)
                {
                    ...
                    {
                        nrTypeParameter.BaseTypes.Add(GetTypeUsage(genericParameterConstraint,
                            genericArgument.DeclaringType));
                    }
                }
        ...
        private class IgnoreAssemblyResolver : PathAssemblyResolver
        {
            private readonly List<string> _errors;

            public IgnoreAssemblyResolver(IEnumerable<string> assemblyPaths, List<string> errors)
                : base(assemblyPaths)
            {
                _errors = errors;
            }
            ...
            public override Assembly? Resolve(MetadataLoadContext context, AssemblyName assemblyName)
            {
                ...
                {
                    _errors.Add($"Could not resolve assembly '{assemblyName.Name}'");
                    return context.LoadFromByteArray([]); // Creates an empty assembly placeholder
                }
            }
        }
    }
